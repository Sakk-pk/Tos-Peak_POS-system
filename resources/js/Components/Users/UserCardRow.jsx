...
export default function UserCardRow({ member, onEdit, onDelete }) {
    ...
    return (
        ...
        >
            ...
            <td className={`px-6 ${classes.cellY}`}>
                <span className={`inline-flex rounded-full px-3 py-1.5 text-sm font-semibold ${member.role === 'Admin' ? 'bg-black text-white shadow-sm shadow-black/10' : 'border border-black/8 bg-white text-black/70'}`}>
                    {member.role}
                </span>
            </td>
        </TableCardRow>
